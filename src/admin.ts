...
createApp({
    ...
    methods: {
        ...
        async loadContractInfo() {
            ...
            const recordCount = Web3.utils.toNumber(await tickets!.methods.totalPriceRecords().call());
            const priceRecords: PriceRecord[] = new Array(recordCount);
            for (let i = 0; i < recordCount; i++) {
                const info = await tickets!.methods.priceRecords(i).call();
                priceRecords[i] = {
                    validTo: Web3.utils.toNumber(info.validTo),
